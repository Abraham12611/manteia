 ...
 * Based on Alchemy SDK and Sui SDK integrations
 */

export function WalletBalanceDisplay() {
  const { balances, refreshBalances, isLoading, hasErrors } = useWalletBalances();
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Real-time balances from connected wallets
            </CardDescription>
          </div>
          <Button
            variant="outline"
        ...
        <div className="space-y-3">
          ...
          {balances.ethereum.loading ? (
            ...
            <div className="space-y-3">
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">{balances.ethereum.native.name}</p>
                        <p className="text-sm text-muted-foreground">{balances.ethereum.native.symbol}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold">{balances.ethereum.native.formattedBalance}</p>
              ...
              {/* ERC-20 Tokens */}
              {balances.ethereum.tokens.length > 0 && (
                <div className="space-y-2">
                  <Button
                    variant="ghost"
                  ...
                  >
                    <span className="text-sm">
                      ERC-20 Tokens ({balances.ethereum.tokens.length})
                    </span>
                    {showEthereumTokens ? (
            ...
            <div className="space-y-3">
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">{balances.sui.native.name}</p>
                        <p className="text-sm text-muted-foreground">{balances.sui.native.symbol}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold">{balances.sui.native.formattedBalance}</p>
                      <p className="text-sm text-muted-foreground">Native</p>
                    </div>
                  </div>
                </CardContent>
              ...
              {balances.sui.tokens.length > 0 && (
                <div className="space-y-2">
                  ...
                  >
                    <span className="text-sm">
                      Sui Tokens ({balances.sui.tokens.length})
                    </span>
                    {showSuiTokens ? (
                      <ChevronUp className="h-4 w-4" />
                    ) : (
